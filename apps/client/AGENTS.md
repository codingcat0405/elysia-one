...
The session token lives in `localStorage` under a namespaced key (`elysia-one.auth_token`), captured from the `set-auth-token` response header on sign-up/sign-in success. To check whether the user is logged in and fetch their profile, call `getSessionUser()` (`src/lib/auth-client.ts`), which wraps `authClient.getSession()`, short-circuits to `null` if there's no token, and collapses every failure (network error, no session, API down) to `null`. **Never read the token directly from `localStorage` outside `lib/auth-token.ts`** — always go through `getAuthToken()` (which guards SSR access and try/catch'd storage access) and let `getSessionUser()` handle the session check. On sign-out, `clearAuthToken()` must always run, even if the server-side session delete fails, so sign-out wraps in a `finally` block.
...
### 4. Auth flow split: `_authed.tsx` guards + hydrates; `__root.tsx` syncs display-only state

**`_authed.tsx` (layout gate):** A pathless layout route wrapping all authenticated screens. **Browser-only mount effect** (not a loader, and not SSR) that awaits `getSessionUser()` behind a `checking` loading state. On any failure (no token, invalid session, network error), it **clears the stored token** and **redirects to `/login`** — this is the _only_ place that invalidates auth. Once the effect resolves, it hydrates the user store with the session. New authenticated screens go under `routes/_authed/` as children so they inherit the guard — don't re-implement a per-route "am I logged in" check.

**`__root.tsx` (display sync):** A separate, **client-only mount effect** in `RootDocument` that calls `getSessionUser()` to populate the user store for the `Header` component on public pages. This effect **never redirects**, **never gates access**, and **never clears the token** — its sole purpose is display correctness. The double `getSessionUser()` call when landing directly on an authenticated route (once in `__root`, once in `_authed`) is deliberate and accepted, not a bug to dedupe.

### 5. Routes stay SSR-enabled, but no auth decision is made during SSR

`login.tsx`, `register.tsx`, and `_authed.tsx` are **not** `ssr: false` (and must **not** be marked `ssr: false`). However, **no authentication check happens on the server side** anymore — the bearer token lives in `localStorage`, which doesn't exist during SSR. The first server render shows a loading state, and the browser-side `_authed` guard validates the session behind that state. `login.tsx` and `register.tsx` render on the server (they have no auth requirement), and `_authed` renders the loading state on first paint, then the app or login redirect once validation completes. Keep SSR enabled so the page shells render quickly and routes load normally without `ssr: false` overhead.

### 5.5. Zustand store is process-global on SSR servers — store writes only from client-side effects

The `useUserStore` (Zustand) is module-scope state, which becomes process-global when the app runs on an SSR server. **Never write to the store from a loader or `beforeLoad` that runs server-side** — every incoming request would see the previous request's user data. Store writes must come only from client-only effects: `_authed.tsx`'s mount effect (gates + hydrates) and `__root.tsx`'s `RootDocument` mount effect (display-only sync). **No loader writes to the store anymore** — all writes are effect-based and client-only. Both effects may read from the store or external state, but only these two places may write to it.
