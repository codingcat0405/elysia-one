...
The app authenticates requests via `Authorization: Bearer <token>` headers. Better Auth's mounted handler (`auth.handler`, wired in `index.ts`'s `.mount()`) still issues and clears the `better-auth.session_token` cookie on sign-in/sign-out (there is no documented flag to suppress it), but this app never reads, sends, or forwards it — the cookie is inert. The only session _read_ anywhere in this codebase is `auth.api.getSession({ headers })` inside `macros/auth.ts`, which reads the bearer token from the real request headers and never touches `cookie` itself. If you find yourself reaching for Elysia's `cookie` context to set, read, or clear anything auth-related, stop — the design has been misunderstood; the bearer flow is the only auth mechanism in use.
