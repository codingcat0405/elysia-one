...
describe('errorMiddleware', () => {
  it('maps an HttpError to its own status and message', () => {
    const set = fakeSet()
    const result = errorMiddleware({ code: 'UNKNOWN', error: new ForbiddenError(), set })
  ...
  it('maps a well-formed VALIDATION error to 400 with its summary/errors', () => {
    const set = fakeSet()
    const error = { message: JSON.stringify({ summary: 'Bad body', errors: ['username required'] }) }
    const result = errorMiddleware({ code: 'VALIDATION', error, set })
    expect(set.status).toBe(400)
    expect(result).toEqual({ message: 'Bad body', errors: ['username required'], status: 400 })
  })
  ...
  it('falls back to a generic validation message instead of throwing on malformed JSON', () => {
    ...
    const result = errorMiddleware({ code: 'VALIDATION', error, set })
    expect(set.status).toBe(400)
    expect(result).toEqual({ message: 'Validation error', errors: [], status: 400 })
  })

  it('maps NOT_FOUND to a generic 404', () => {
    const set = fakeSet()
    const result = errorMiddleware({ code: 'NOT_FOUND', error: new Error('unused'), set })
  ...
  it('maps anything else to a generic 500 without leaking the internal error text', () => {
    const set = fakeSet()
    const error = new Error('duplicate key value violates unique constraint "user_email_unique"')
